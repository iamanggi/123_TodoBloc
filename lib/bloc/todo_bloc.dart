...
class TodoBloc extends Bloc<TodoEvent, TodoState> {
  TodoBloc() : super(TodoLoaded(todos: [], selectedDate: null)) {
    //menggunakan TodoEventAdd dan menghubungkan dengan TodoLoaded
    on<TodoEventAdd>((event, emit) {
      final currentState = state;
      if (currentState is TodoLoaded){
        final List<Todo> updatedTodos = List.from(currentState.todos); //untuk menampung list
        updatedTodos.add(
          Todo(
            title: event.title, date: event.date, isCompleted: false) //data yang ada dimodel
        );
        emit(TodoLoaded(
          todos: updatedTodos, 
